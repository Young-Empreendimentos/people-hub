...
export default function Descontos() {
  const { data: descontos = [], isLoading } = useQuery({
    queryKey: ["rh_folha_descontos_meses"],
    queryFn: async () => {
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Visualize por mês os descontos lançados nas folhas mensais.
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <Card><CardContent className="p-6 text-center text-muted-foreground">
          Nenhum desconto lançado ainda.
        </CardContent></Card>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {meses.map((m) => (
            <Link key={m.mes} to={`/descontos/${m.mes}`}>
