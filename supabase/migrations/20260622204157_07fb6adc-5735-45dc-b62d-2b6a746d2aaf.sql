...
RETURNS TABLE(
  ...
  status text, funcionario_id uuid, funcionario_nome text
)
...
BEGIN
  ...
  RETURN QUERY
  SELECT
    au.id, au.email::text, ur.role, au.created_at,
    COALESCE(up.nome, '') AS nome,
    COALESCE(ur.status, 'pendente') AS status,
    ur.funcionario_id,
  ...
  LEFT JOIN public.rh_funcionarios f ON f.id = ur.funcionario_id
